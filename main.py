...
def is_valid_placement(existing_elements, new_x, new_y, new_width, new_height, min_distance=15):
    def recheck(index):
        ...
        x, y, width, height = existing_elements[index]

        # Checking if the current placement is valid or nah
        if (new_x + new_width + min_distance <= x or 
            new_x >= x + width + min_distance or 
            new_y + new_height + min_distance <= y or 
            new_y >= y + height + min_distance):
            return recheck(index + 1)
...
class Viewport(tk.Tk):
    ...
    def zoom_image(self):
        ...
        # Get the current center of the viewport in canvas coordinates
        ...
        img_center_x = canvas_center_x / self.zoom_factor
        img_center_y = canvas_center_y / self.zoom_factor
        ...
        # Calculate the new canvas center position in image coordinates
        new_canvas_center_x = img_center_x * self.zoom_factor
        new_canvas_center_y = img_center_y * self.zoom_factor
        
        # Adjust the canvas view to keep the center at the same position
        self.canvas.xview_moveto((new_canvas_center_x - canvas_width / 2) / new_size[0])
        self.canvas.yview_moveto((new_canvas_center_y - canvas_height / 2) / new_size[1])
